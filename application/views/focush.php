...
$after = $cari->id_cerita - 1;
$before = $cari->id_cerita + 1;
?> 
<div class="row">
        <div class="col-8">
...
<div class="grid">
  <div class="next">
  <?php if($before<=$row->id_cerita){ ?>
    <a href="<?= site_url('Ketawa/focus/'.$before)?>"><< << sebelum </a>
  <?php } ?>
  


  <?php if($after>=0){ ?>
    <a href="<?= site_url('Ketawa/focus/'.$after)?>">berikut >> >></a>
  <?php } ?>
</div>
</div>

<div>
  <form action="<?php echo site_url('Ketawa/submitComment/X/'.$cari->id_cerita)?>" id="usrform" class="grid" method="post">
